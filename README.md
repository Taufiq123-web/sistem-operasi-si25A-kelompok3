...
## 📝 Kesimpulan
Melalui praktikum ini, kami memperoleh pengalaman yang sangat berharga dalam mempelajari dasar-dasar administrasi server Linux. Kami tidak hanya memahami cara membuat Virtual Machine di VMware dan menginstal Debian 13 dalam mode teks (headless), tetapi juga mengetahui bagaimana menginstal aplikasi pendukung, mengonfigurasi web server Nginx, serta melakukan port forwarding agar server dapat diakses dari komputer host. Selama proses pengerjaan, kami menghadapi beberapa kendala yang membuat kami harus lebih teliti dalam setiap langkah konfigurasi dan mencari solusi secara bersama-sama. Pengalaman tersebut membantu kami memahami bahwa membangun sebuah server memerlukan ketelitian, pemahaman konsep, serta kerja sama yang baik antaranggota kelompok. Secara keseluruhan, praktikum ini menambah wawasan dan keterampilan kami dalam mengelola server Linux, sekaligus memberikan gambaran nyata mengenai penerapan administrasi server yang akan bermanfaat untuk pembelajaran maupun dunia kerja di masa mendatang.
